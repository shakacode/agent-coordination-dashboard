...
(`import Ajv2020 from "ajv/dist/2020"`) and `validateFormats: false`: the schema's `date-time` and
`uri` formats are not installed in Ajv's core, and the `timestamp` pattern plus the shared URL
validators already cover them.
